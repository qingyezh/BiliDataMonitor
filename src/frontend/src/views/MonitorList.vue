  ...
  <div>
      ...
      <el-table :data="tasks" stripe v-loading="loading">
        ...
        <el-table-column label="类型" width="90" align="center">
          <template #default="{ row }">
            <el-tag :type="row.task_type === 'up' ? 'primary' : 'success'" size="small">
              {{ row.task_type === 'up' ? 'UP主' : '视频' }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="showCreateDialog" title="新增监测任务" width="460px">
      <el-form label-width="90px">
        ...
        <el-form-item :label="createForm.task_type === 'up' ? 'UP主UID' : '视频BV号'">
          <el-input v-model="createForm.target" :placeholder="createForm.task_type === 'up' ? '输入UP主UID（如 3546597658987119）' : '输入视频BV号（如 BV1aWuG6mEyN）'" />
        </el-form-item>
